...
| **Audacity** (kiểm soát chi tiết)                   | Muốn tự tay chỉnh mức độ                                        | Chọn đoạn _chỉ có_ tiếng ồn, không giọng → `Effect > Noise Reduction > Get Noise Profile` → chọn (Ctrl+A) cả track → `Effect > Noise Reduction` → OK              |
...
rồi copy cả 2 vào thư mục `python-backend/model/` (hoặc `model/`) trong repository.

> Phân biệt 2 thư mục Drive: `ApplioBackup/` chỉ chứa checkpoint để _tiếp tục train_ nếu bị ngắt
...
**Backup trong lúc train diễn ra tự động.** Miễn là bạn đã chạy cell _Sync with Google Drive_
...
`Save frequency` epoch). Cách tiếp tục: mở lại notebook → chạy lại _Install Applio_ + _Sync with
Google Drive_ → sang tab Train, nhập **đúng tên model cũ**, **đúng sample rate cũ** → tăng
...
Chuyển vào thư mục `python-backend/`:
...
là xong — **giữ cửa sổ terminal này mở** trong lúc sử dụng. Kiểm tra nhanh: mở
...
3. Trong mục **Nhà cung cấp giọng đọc**, chọn **"Giọng của tôi (Local RVC)"**.
...
## Một lưu ý về bản quyền giọng nói
